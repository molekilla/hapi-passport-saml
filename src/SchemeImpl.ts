...
export const SchemeImpl = (saml: HapiSaml, options: HapiSamlOptions) => (server: any, settings: SchemeConfig) => {
    const cookieOptions = {
        encoding: 'iron',
        path: '/',
    ...
    settings.cookie = settings.cookie || 'hapi-passport-saml-' + options.config.cookieName;
